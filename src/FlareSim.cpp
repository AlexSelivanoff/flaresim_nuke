...
static const char *const kPresetNames[] = {
    "Custom",           "Full Frame",        "Super 35", "APS-C Canon",
    "APS-C Nikon/Sony", "Micro Four Thirds", nullptr};
...
static void cluster_sources(std::vector<BrightPixel> &sources, int radius_px,
                            int fmt_w, float tan_half_h) {
  if (radius_px <= 0 || (int)sources.size() < 2)
    return;
...
public:
  ...
  // Input channel selection (which layer to read lights from)
  ChannelSet input_channelset_;
  ...
  explicit FlareSim(Node *node)
      : Iop(node), lens_file_(""), flare_gain_(1000.0f), ray_grid_(100),
        ...
        haze_radius_(0.15f), haze_blur_passes_(3), starburst_gain_(0.0f),
  ...
  void rebuild_pair_ui() {
    ...
    if (fov_use_sensor_) {
      ...
      const float aspect = (pending_fmt_w_ > 0 && pending_fmt_h_ > 0)
                               ...
                               : 16.0f / 9.0f;
      fov_v = fov_auto_v_ ? 2.0f * std::atan(std::tan(fov_h * 0.5f) / aspect)
    ...
    if ((int)active_pairs.size() > MAX_PAIRS_UI) {
      fprintf(stderr,
              "FlareSim: %d active pairs, displaying first %d in Pairs tab.\n",
    ...
    for (int k = 0; k < n; ++k) {
      ...
      snprintf(pair_labels_[k], sizeof(pair_labels_[k]),
               "Pair %d  (surf %d <-> surf %d)", k, p.surf_a, p.surf_b);
  ...
  void knobs(Knob_Callback f) override {
    ...
    static const char *const kJitterModes[] = {"Off", "Stratified", "Halton",
                                               nullptr};
    Enumeration_knob(f, &pupil_jitter_, kJitterModes, "pupil_jitter",
                     "Pupil Jitter");
    Tooltip(f, "Entrance-pupil sampling pattern.\n"
               "Off: regular NxN grid — fastest, but visible dot pattern at "
               "low ray counts.\n"
               "Stratified: one randomly-placed sample per grid cell — breaks "
               "up the dot pattern.\n"
               "Halton: quasi-random low-discrepancy sequence — deterministic, "
               "very even coverage.");

    Int_knob(f, &jitter_seed_, "jitter_seed", "Jitter Seed");
    Tooltip(f,
            "Integer seed for the stratified noise pattern. "
            "Only used when Auto Seed is off and Pupil Jitter is Stratified.");
    Bool_knob(f, &jitter_auto_seed_, "jitter_auto_seed", "Auto Seed");
    Tooltip(
        f,
        "When on (the default), the jitter seed is automatically set to the "
        "current "
        "frame number, so sample positions change on every frame without any "
        "manual "
        "keyframing — equivalent to animating Jitter Seed = frame.\n\n"
        "Turn off to use the fixed Jitter Seed value, which gives a repeatable "
        "pattern regardless of frame number (useful for still renders or when "
        "you "
    ...
    Tooltip(f, "Maximum number of bright sources passed to the GPU per frame. "
               ...
               "Reduce this if you experience GPU crashes on frames with many "
               "sources.");
    Int_knob(f, &downsample_, "downsample", "Downsample");
    Tooltip(f, "Sample every Nth pixel in each dimension when extracting "
               "bright sources. "
               "4 means 1/16th of pixels are used. Lower = more sources = "
               "brighter but slower.");
    Int_knob(f, &cluster_radius_, "cluster_radius", "Cluster Radius");
    Tooltip(
        f, "Merge nearby bright sources that fall within this radius (in "
           "pixels) of each other "
           "into a single, stronger source at their luma-weighted centroid.\n\n"
           "Useful when a large bright area (e.g. a car headlight, the sun, or "
           "a bokeh highlight) "
           "is detected as many adjacent sources that produce near-identical, "
           "overlapping ghosts. "
           "Clustering reduces GPU work and eliminates the slight banding that "
           "those duplicate "
           "ghosts can produce.\n\n"
           "0 = off. A value roughly equal to the physical size of your "
    ...
    Tooltip(f, "When enabled, FOV is computed from sensor dimensions and focal "
               "length.");
    Enumeration_knob(f, &sensor_preset_, kPresetNames, "sensor_preset",
                     "Sensor Preset");
    if (!fov_use_sensor_)
      SetFlags(f, Knob::DISABLED);
    Tooltip(f, "Common sensor size presets. Selecting a preset fills Sensor "
               "Width and Height.");
    ...
    if (fov_use_sensor_)
      SetFlags(f, Knob::DISABLED);
    Tooltip(f,
            "Derive vertical FOV from horizontal FOV and image aspect ratio.");
    Float_knob(f, &fov_v_deg_, "fov_v", "FOV V (deg)");
    if (fov_auto_v_ || fov_use_sensor_)
      SetFlags(f, Knob::DISABLED);
    Tooltip(f, "Vertical field of view in degrees. Ignored when Auto FOV V is "
               "enabled.");
    ...
    Tooltip(
        f,
        "0 = circular aperture. 3–16 = regular polygon — affects ghost shape.");
    Float_knob(f, &aperture_rotation_, "aperture_rotation",
               "Aperture Rotation");
    SetRange(f, -180.0, 180.0);
    Tooltip(f, "Rotation of the aperture polygon in degrees.");

    Divider(f, "Spectral");
    static const char *const kSpecNames[] = {"3 (R/G/B)", "5",  "7",
                                             "9",         "11", nullptr};
    Enumeration_knob(f, &spectral_idx_, kSpecNames, "spectral_samples",
                     "Spectral Samples");
    Tooltip(f,
            "Number of wavelength samples for chromatic aberration. "
    ...
    Tooltip(
        f,
        "Post-splat blur radius as a fraction of the image diagonal. 0 = off.");
    ...
    Tooltip(f, "Veiling glare intensity multiplier. 0 = off. "
               "Haze is a wide soft glow splatted from each bright source into "
               "haze.rgb. "
    ...
    Tooltip(
        f,
        "Number of box-blur passes for the haze. 3 approximates a Gaussian.");

    Divider(f, "Starburst");
    Float_knob(f, &starburst_gain_, "starburst_gain", "Starburst Gain");
    Tooltip(
        f,
        "Diffraction spike intensity multiplier. 0 = off.\n\n"
        ...
        "Spikes are only visible with a polygonal aperture (Aperture Blades >= "
        "3). "
        "A circular aperture (0 blades) produces Airy rings instead.\n\n"
        "The pattern is wavelength-dependent: blue light diffracts less than "
        "red, "
    ...
    Enumeration_knob(f, &output_mode_, kOutputModes, "output_mode",
                     "Output Mode");
    Tooltip(f,
            "Separate channels: flare data written only to flare.rgb and "
            "source.rgb.\n"
            "Flare as RGB: flare also written to the main RGB channels (input "
            "RGB discarded).\n"
            "Sources Only: shows the detected source map in the main RGB "
            "channels and skips "
            "all rendering (ghosts, haze, starburst). Use this to quickly tune "
            "Threshold, "
    ...
    for (int k = 0; k < MAX_PAIRS_UI; ++k) {
      Bool_knob(f, &pair_enabled_[k], s_pair_knob_names.names[k],
                pair_labels_[k]);
  ...
  void do_compute() {
    ...
    // Resolve input channels: take the first 3 channels from the
    // user-selected channel set (e.g. rgba -> R,G,B; diffuse -> dR,dG,dB).
    Channel in_chan[3] = {Chan_Red, Chan_Green, Chan_Blue};
    {
      int ci = 0;
      foreach (z, input_channelset_) {
        if (ci < 3)
          in_chan[ci++] = z;
      }
    }
    ...
    for (int iy = y0; iy < y1; ++iy) {
      input0().get(iy, x0, x1, input_channelset_, input_row);
      if (has_mask)
      ...
      const float *rp = input_row[in_chan[0]];
      const float *gp = input_row[in_chan[1]];
      const float *bp = input_row[in_chan[2]];
    ...
    // are detected (proportional to 1/ds²), so scale gain up by
    // (ds_preview/ds_final)² to keep overall brightness consistent. Note: this
    // compensation only accounts for downsample — if Max Sources is also lower
    ...
    cfg.spectral_samples =
        kSpecCounts[std::max(0, std::min(eff_spectral_idx, 4))];
    cfg.pupil_jitter = pupil_jitter_;
    cfg.pupil_jitter_seed =
        jitter_auto_seed_ ? (int)outputContext().frame() : jitter_seed_;

    std::vector<GhostPair> active_pairs;
    std::vector<float> area_boosts;
    filter_ghost_pairs(lens_, sensor_half_w, sensor_half_h, cfg, active_pairs,
                       area_boosts);
    ...
    // Apply per-pair user toggles: keep only pairs the user has enabled.
    {
      ...
      for (int i = 0; i < (int)active_pairs.size(); ++i) {
        ...
        const bool enabled =
            (idx < 0 || idx >= MAX_PAIRS_UI) || pair_enabled_[idx];
        if (enabled) {
    ...
    if (eff_max_sources > 0 && (int)sources.size() > eff_max_sources) {
      std::partial_sort(
          sources.begin(), sources.begin() + eff_max_sources, sources.end(),
          [](const BrightPixel &a, const BrightPixel &b) {
    ...
    launch_ghost_cuda(lens_, active_pairs, area_boosts, sources, sensor_half_w,
                      sensor_half_h, out_r.data(), out_g.data(), out_b.data(),
                      w, h, pending_fmt_w_, pending_fmt_h_, fmt_x0_in_buf,
                      fmt_y0_in_buf, cfg, gpu_cache_, &pending_cuda_error_);
    ...
    if (starburst_gain_ > 0.0f && !sources.empty()) {
      ...
      if (starburst_psf_.empty() || aperture_blades_ != last_sb_blades_ ||
      ...
      render_starburst(starburst_psf_, sb_cfg, sources, th, tv, sb_r.data(),
                       sb_g.data(), sb_b.data(), w, h, pending_fmt_w_,
                       pending_fmt_h_, fmt_x0_in_buf, fmt_y0_in_buf);
    }
  ...
  void engine(int y, int x, int r, ChannelMask channels, Row &row) override {
    ...
    // Pass non-managed channels through from input.
    {
      ...
      pass_through -= kFlareRed;
      pass_through -= kFlareGreen;
      pass_through -= kFlareBlue;
      pass_through -= kSourceRed;
      pass_through -= kSourceGreen;
      pass_through -= kSourceBlue;
      pass_through -= kHazeRed;
      pass_through -= kHazeGreen;
      pass_through -= kHazeBlue;
      pass_through -= kStarburstRed;
      pass_through -= kStarburstGreen;
      pass_through -= kStarburstBlue;
      if (output_mode_ != 0) {
    ...
    const bool valid_cache =
        !ghost_r_.empty() && cache_y >= 0 && cache_y < cache_height_;
  ...
  void _request(int x, int y, int r, int t, ChannelMask channels,
                int count) override {
    // Always request the full frame from input 0 for source detection.
    input0().request(info_.x(), info_.y(), info_.r(), info_.t(),
                     input_channelset_, count);

    // If a mask is connected, request its alpha over the full frame too.
    if (input(1))
      input(1)->request(info_.x(), info_.y(), info_.r(), info_.t(), Mask_Alpha,
                        count);

    // Pass non-managed channels through for the requested region only.
    ChannelSet passthru(channels);
    passthru -= input_channelset_;
    passthru -= kFlareRed;
    passthru -= kFlareGreen;
    passthru -= kFlareBlue;
    passthru -= kSourceRed;
    passthru -= kSourceGreen;
    passthru -= kSourceBlue;
    passthru -= kHazeRed;
    passthru -= kHazeGreen;
    passthru -= kHazeBlue;
    passthru -= kStarburstRed;
    passthru -= kStarburstGreen;
    passthru -= kStarburstBlue;
    if (passthru)
...
const Iop::Description FlareSim::d("FlareSim", "Filter/FlareSim", build);
